...
// PIPE PROPERTIES
const PIPE_WIDTH: number = 10;
const PIPE_COLOR: string = "green";
const PIPE_FREQ: number = 150; // Controls how often pipes are generated
// BOX PROPERTIES
const BOX_COLOR: string = "red";
const BOX_SIZE: number = 30;
const BOX_INIT_X: number = 10;
const BOX_INIT_Y: number = GAME_HEIGHT - 120;
const GRAVITY: number = -0.05;
// SCORE PROPERTIES
const SCORE_SIZE: string = "20px";
const SCORE_FONT: string = "Consolas";
const SCORE_COLOR: string = "red";
const SCORE_X: number = 369;
const SCORE_Y: number = 248;
...
function Score(size, font, color, x, y) {
  ...
  this.update = function() {
    let scoreText = "Score: " + gameArea.frame.toString();
    context = gameArea.context;
...
function Pipes() {
  ...
  this.hasCrashed = function() {
    for(i=0; i<this.pipeArray.length; i++){
      let boxX  = birdBox.x,
          pipeX = this.pipeArray[i][0].x,
          boxPassed  = boxX > pipeX,
          notInRange = boxX + BOX_SIZE < pipeX;
      
      if(boxPassed || notInRange) {
        continue;
      }

      let topPipe = this.pipeArray[i][topIdx],
          botPipe = this.pipeArray[i][botIdx],
          topPipeBottom = topPipe.y - topPipe.height,
          botPipeTop    = botPipe.y;
      let boxTop = birdBox.y,
          boxBot = birdBox.y - BOX_SIZE;
      let hitTopPipe = boxTop > topPipeBottom,
          hitBotPipe = boxBot < botPipeTop;
...
function newPipes() {
  ...
  let bottomHeight  = randomInt(minHeight, maxHeight),
      gap           = randomInt(minGap, maxGap);
      
  let bottomX   = gameArea.canvas.width,
      bottomY   = bottomHeight,
      topX      = bottomX,
      topY      = GAME_HEIGHT,
      topHeight = GAME_HEIGHT - bottomHeight - gap;
  
   let bottom = new BaseItem(PIPE_WIDTH, bottomHeight, PIPE_COLOR, bottomX, bottomY),
       top    = new BaseItem(PIPE_WIDTH, topHeight, PIPE_COLOR, topX, topY);
...
function updateGame() {
  ...
  gameArea.clear();
  ++ gameArea.frame; 
  let canSpawnPipe = (gameArea.frame % PIPE_FREQ) === 0;
